...
6. [ ] We have a minor issue where when we generate the static build, ffmpeg sends a bunch of warnings to the console (probably to stderr, but that should be confirmed). this likely happens while fetching metadata from videos.  i've been testing in the ~/Documents/Magic directory which produces lots of these.  use that if needed. i don't want any ffmpeg output to show up.
7. [ ] Get serious about the UI complete with vim-like shortcut keys (j/k, space, H/L, /, maybe ctrl-p, ctrl-f/b, ctrl-d/u, -, etc.)
