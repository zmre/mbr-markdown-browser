...
| `--no-review` | Disable `data-mbr-line` source lines on block elements; server/GUI only | `false` |
...
The build produces **six bundles** — one main bundle plus five lazy chunks, each with its own vite config:
...
| `mbr-review.min.js` | `vite.review.config.ts` | `<mbr-review-panel>` + `<mbr-review-form>`: the review list, the note form and the export formatter (~51 kB min / ~15 kB gz) | Lazily, the first time a note is written or the list is opened. **Excluded from static builds** — see `REVIEW_CHUNK_ROUTE` |
...
| `markdown.rs` | pulldown-cmark markdown parsing with YAML frontmatter extraction. Three passes: `collect_events_and_headings` (headings, `--- {attrs}`, task rewrite — the only place parser byte ranges exist), `process_all_events` (links, media, oembed), and `mark_incomplete_blocks` (TK/TODO highlighting). The third needs source lines the first one has and the second one destroys, so `TextLines` carries them across by **event index** — sound only because `process_event` is a strict 1:1 map, which is now stated as an `# Invariant` and pinned by a `debug_assert_eq!`. Recording a line per text run is enough because `SoftBreak`/`HardBreak` end a merge, so a merged `Event::Text` never spans a source line *outside code blocks* — which is also why `mark_incomplete_blocks` skips code (a `code_depth` guard; a fence inside a list item used to be wrapped) and image alt text (`html::raw_text` silently drops injected `Event::Html`). Anchors are `id="mbr-marker-{line}"`, at most one per source line. A **second, independent** table, `BlockLines`, carries source lines for the six block starts `is_review_block_start` names (that function is the single definition of the set; `html.rs`'s arms cite it) so the writer can emit `data-mbr-line`. Separate from `TextLines` because the gates are separate — `mark_incomplete` vs `review_enabled` — and one monotone cursor cannot serve two consumers. It is the **only** table that has to survive a pass that is not 1:1: `mark_incomplete_blocks` is all `output.push` save one `output.insert(top.start_idx + 1, …)`, so it *rebuilds* the table against its own output (`remapped.record(output.len(), …)` at the top of the loop, `remapped.shift_from` right after the insert). The shifting case is real and default-on — a fenced code block in a tight list item, whose `<pre>` is recorded before the item's own wash span is inserted behind it. `pop_event`'s truncation is load-bearing here, not belt-and-braces: both pass-1 rewrites pop a recorded `Start(Tag::Paragraph)` |
...
| `html.rs` | Custom HTML output for pulldown-cmark with section support. Also emits `data-mbr-line` from `HtmlConfig::block_lines` (**empty means the feature is off**, which is what every non-server caller gets). **The writer pulls from `self.iter` in two loops — `run()` and `raw_text()`, the latter draining an image's inner events into its `alt` attribute — so both MUST go through `next_event()`**, the single place `event_index` is maintained: a counter incremented only in `run()` desynchronises permanently after the first image on the page and every later `data-mbr-line` names the wrong line, with well-formed HTML and no error. `block_line_index_survives_image_alt_text` pins it. On a heading the attribute is written after the level and **before** the author's `id`/`classes`/`attrs`, because an HTML parser keeps the first of a duplicate pair; on a code block it goes on the `<pre>`, in all three shapes |
...
- `mbr-review.ts` - Review-notes trigger and the in-document half (`<mbr-review>`, main bundle). Emitted by `_footer.html` under `{% if server_mode and review_enabled %}` — **not** `_nav.html`, because the feature deliberately has no header button: `r` on a selection, `R` for the list, and a FAB that appears only once a note exists. Owns everything the chunk may not hold (the store, `readSourceLines`, the `site.json`-derived URL resolver) and injects it as Lit properties. `isOpen` covers the **form** as well as the panel — a form's `<select>` and buttons are not `isInputTarget`, so without it a bare `j` scrolls the page behind an open form — and stays false when the feature is off, since an `isOpen` reporting true behind an invisible overlay would make `isModalOpen()` suppress every bare-letter shortcut on the page. Both keys are **open-only, never toggles**, for `mbr-tasks.ts`'s reason: once the form is open its textarea makes `isInputTarget` true, so `r` must reach it as a literal character. `mbr-review-edit` is dispatched cancelable by the panel and `preventDefault()` here is what *claims* it — without that call two forms open at once, each with its own idea of what is being edited. The re-anchoring pass is guarded on `isConnected` because `scheduleIdleTask` cannot be cancelled and `reviewRoot()` is a document-level query, so a disconnected element would otherwise still write re-anchored lines to the store.
- `review-store.ts` - `localStorage` under a `mbr_review_notes`-prefixed key scoped to the served repository via `shared.ts`'s `getRepoId()` (main bundle). **The only module in the review feature allowed to name `localStorage`, and it is on `edit-token.test.ts`'s allowlist** — that scan is a plain substring match *per module*, so routing storage through a shared wrapper would make every consumer invisible to it and leave one file standing in for all of them; hence the direct calls here and the pure parse/coerce/serialize half in `review/note-model.ts`. It lives outside `review/` on purpose: the lazy chunk must not import it, and a stateful module inside the chunk's own directory invites exactly that. Every mutation is a read-modify-write keyed on the note's `id` rather than a write of the cached array, so two windows on the same repo cannot silently clobber each other, and a `storage` listener keeps both live. A store written by a **newer** mbr (envelope version) goes read-only rather than being overwritten with fields this build cannot see. Contrast `edit-token.ts`: notes are the reader's own words about a document already on screen, not a credential, and durability across a live reload is the entire feature.
- `review/` - The lazy `mbr-review.min.js` chunk plus the pure modules both bundles share. **Nothing here may import `shared.ts`, `review-store.ts` or `task-toggle.ts`** — a second copy of any of them is a second cache and a second subscriber set, and a note saved through one would be invisible to the other; everything stateful arrives as an element property. `types.ts` holds `TYPE_DEFS` (the six types in `zmre/pwnvim`'s order, which is the wire order and is pinned by nothing else, since `note-order.ts` does not sort by type) and `REVIEW_ANCHOR_PREFIX`, which is **here rather than in either element** because the marker layer writes the id in the main bundle and the panel builds `#mbr-review-<id>` hrefs in the chunk — two literals is exactly the drift that makes a deep link land silently at the top of the page. `markers.ts` is main-bundle: its `.mbr-review-marker` is **textless**, the glyph coming from a `::before` rule, so it stays out of selections, `textContent` and `find-in-page.ts`'s index (the trap `mbr-heading-enhancer.ts` documents for the `#` permalink); the quote's wash is `CSS.highlights` rather than an injected `<mark>` because wrapping splits text nodes and invalidates every cached node reference — and the `::highlight(mbr-review)` rules therefore live in `theme.css`, not in any `static styles`, since the registry is document-scoped and the ranges are in the light DOM. `reanchor.ts` searches the **rendered** text verbatim (the quote is stored in `TextIndex` form), so drift correction needs no network and works without `--edit`; a `PREFIX_LEN` fallback relocates a quote whose tail changed, and a quote that is gone leaves the note in place flagged `lost` — staleness is a badge, never a deletion. `export-format.ts` is pure and clock-free so the markdown can be asserted verbatim.
- `anchored-popover.ts` - Viewport-clamped placement for a card hanging off an element, shared by `<mbr-footnote-preview>`, the mini graph's node hover card and the review markers — the third consumer is where a third copy stopped being cheaper. Pure apart from two style writes, which is not incidental: happy-dom reports every `getBoundingClientRect()` as zeros, so its users can only be tested through `positionRects`. Safe for a lazy chunk to import (no module state, no fetches, no caches).
...
- `nav-title.ts` - Ellipsis + tooltip for the header page title (main bundle, no custom element — it enhances the server-rendered `_nav.html` markup in place). The clamp itself is CSS; this module exists because CSS cannot tell whether text is *actually* clipped, and a tooltip repeating a title already fully on screen is noise. It compares `scrollWidth` to `clientWidth` (re-checked via `ResizeObserver`, and once more after `document.fonts.ready`, since fonts land after first paint and change the measurement) and sets Pico's `data-tooltip` plus `tabindex="0"` only while clipped. Pico's tooltip is styled on **both** `:hover` and `:focus`, which is the whole reason for using it over a native `title` attribute: `title` covers only hover, and hover is exactly what a touch device does not have. The clamp lives on the inner `<strong>`, never on the `<li>` that carries `data-tooltip` — an `overflow: hidden` ancestor would cut off Pico's absolutely-positioned bubble. `theme.css` also gives that bubble `width: max-content`, without which it shrink-to-fits inside the deliberately-narrowed `<li>` and renders one word per line.
...
`linux/mbr.desktop` plus `mbr-icon.png` are installed by the Nix package into
`share/applications/` and `share/icons/hicolor/256x256/apps/`. Without a desktop entry an
application launcher cannot tell `mbr` is a GUI program and runs it through a terminal,
producing a console window alongside the browser window. `Path=/` is deliberate: it makes
`main.rs::needs_folder_picker` open the folder picker rather than let `mbr`'s default path
argument (`.`) point at whatever directory the launcher happened to be in. The entry is
only visible to a launcher once mbr is on an `$XDG_DATA_DIRS` path (`nix profile install`),
not from a bare `nix build`.

## EGL on non-NixOS hosts

WebKitGTK 2.52's web process `CRASH()`es if `eglGetDisplay` returns `EGL_NO_DISPLAY`, and
no environment variable opts out (`WEBKIT_DISABLE_DMABUF_RENDERER` and
`WEBKIT_DISABLE_COMPOSITING_MODE` were both tried; the abort is upstream of either, in
`WebProcess::initializePlatformDisplayIfNeeded`). Nix's libglvnd finds vendor ICDs under
`/run/opengl-driver/...` on NixOS but nowhere usable on other distributions — the host's
`libEGL_mesa.so` cannot be loaded from a Nix process, since its `libgallium-*.so` is not
on a Nix binary's search path and is built against a different glibc. `flake.nix` therefore
**appends** nixpkgs' Mesa as a last-resort EGL vendor (`glvndVendorDefaults` /
`mesaEglVendorDir`), in both the package wrapper and the devShell. Appended, never
substituted, so a host driver still wins where there is one. Costs ~805 MiB of closure on
a host with no Mesa in its store; near-zero on NixOS.
